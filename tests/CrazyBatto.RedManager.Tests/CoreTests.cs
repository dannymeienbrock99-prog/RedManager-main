...
public sealed class ApiCompatibilityTests
{
    ...
    [Fact]
    public void Highest_semantic_version_is_selected_without_flag()
    {
        var releases = new[]
        {
...
public sealed class SafeArchiveInstallerTests : IDisposable
{
    ...
    public async Task Valid_wrapped_redloader_layout_is_installed()
    {
        ...
        Assert.Contains("Mods/TestMod.dll", installed, StringComparer.OrdinalIgnoreCase);
        Assert.Contains("Mods/TestMod/manifest.json", installed, StringComparer.OrdinalIgnoreCase);
