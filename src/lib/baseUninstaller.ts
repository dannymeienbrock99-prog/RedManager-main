...
export class BaseUninstaller {
    ...
    public preserveFilesWhenFolderExists: string | null = null;
    ...
    public async uninstall(): Promise<void> {
        ...
        const preserveSharedFiles = this.preserveFilesWhenFolderExists
            ? await fs.exists(await this.getFilePath(this.preserveFilesWhenFolderExists))
            : false;

        if (!preserveSharedFiles) {
